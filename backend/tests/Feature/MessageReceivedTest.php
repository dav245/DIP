...
class MessageReceivedTest extends TestCase
{
    ...
    public function test_user_can_get_received_messages()
    {
        $response = $this->actingAs($this->user)->getJson(route('message.received'));

        $response->assertStatus(200);

        $this->assertCount(1, $response->json('messages'));

        $response->assertJsonPath('messages.0.id', $this->messageReceived->id);
    }

    public function test_not_logged_in_user_cannot_get_received_messages()
    {
        $response = $this->getJson(route('message.received'));
